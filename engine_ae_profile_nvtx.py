...
def compute_loss(
    ...
    ######################## chamfer loss ########################
    assert data_dict["points"].shape[1] == (
    ...
    if config.loss.cd_sample_type == "rand":
        ...
        xyz_lp = rearrange(
            gaussians_lp[..., :3],
            "b n (m s) d -> (b m) n s d",
            m=config.loss.sample_cd_iter,
            s=config.loss.num_samples_cd,
        )  # [B*m, N=2048, S, 3]
        xyz_gt = rearrange(
            data_dict["points"][:, 2048 : 2048 + config.loss.sample_cd_iter * 8192, :],
            "b (m n s) d -> (b m) n s d",
            m=config.loss.sample_cd_iter,
            s=config.loss.num_samples_cd,
        )  # [B*m, N=2048, S=4, 3]
    ...
    if config.loss.emd_weight != 0.0:
        loss_lp_emd0 = EMD(
            ...
            data_dict["points"][:, :2048, :],
            origin=config.loss.emd_origin,
            scale=config.loss.emd_scale,
            check_range=(epoch == 0),
        ).mean()
        loss_lp_emd2 = EMD(
            rearrange(xyz_lp[:, :, :4, :], "b n s d -> b (n s) d"),
        ...
        loss_lp_emd_render = EMD(
            xyz_render[:, rand_idx, :],
            data_dict["points"][:, -8192:, :],
            origin=config.loss.emd_origin,
            scale=config.loss.emd_scale,
            check_range=(epoch == 0),
        ).mean()
    ...
    if config.loss.imagenet_background:
        bg_color = torch.tensor(
            IMAGENET_DEFAULT_MEAN, dtype=torch.float32, device=device
        )
    else:
    ...
    images_gt = images_gt * masks_gt + bg_color.view(1, 1, 3, 1, 1) * (
        1 - masks_gt
    )  # [B, V, 3, H, W]
    ...
    loss = (
        loss_kl * config.loss.kl_weight
        + (loss_lp0 + loss_lp1 + loss_lp2 + loss_lp_render) * config.loss.cd_weight
    )
    loss += (loss_lp_emd0 + loss_lp_emd2 + loss_lp_emd_render) * config.loss.emd_weight
    # Hack to receive gradients
    loss += model.img_encoder.backbone.mask_token.sum() * 0.0
    ...
    dmat = torch.linalg.norm(
        xyz[:, :, None, :] - xyz[:, None, :, :], dim=-1
    )  # [B*N, S, S]
    dmat_avg = torch.sum(dmat, dim=(1, 2), keepdim=True) / (
        dmat.shape[-1] ** 2 - dmat.shape[-1]
    )  # [B*N, 1, 1]
    ...
    if epoch > config.loss.render_epochs:
        loss += (
            loss_render_rgb * config.loss.rgb_weight
            + loss_render_mask * config.loss.mask_weight
            + loss_render_depth * config.loss.depth_weight
        )
    ...
    if epoch > config.loss.lpips_epochs:
        ...
        loss_lpips = model.lpips_loss(
            ...
            F.interpolate(
                rearrange(render_dict["images"], "b v d h w -> (b v) d h w") * 2 - 1,
                (256, 256),
                mode="bilinear",
                align_corners=False,
            ),
...
def train_one_epoch(
    ...
    for data_iter_step, data_dict in enumerate(
        ...
        with torch.amp.autocast(device_type="cuda", enabled=config.train.use_fp16):
            ...
            if config.loss.cd_sample_type == "rand":
                num_samples_cd_total = (
                    config.loss.num_samples_cd * config.loss.sample_cd_iter
                )
        ...
        loss, render_dict = compute_loss(
            data_dict,
            outputs_dict,
            gaussians_lp.float(),
            gaussians_render.float(),
            model,
            config,
            epoch,
            device,
        ...
        psnr = compute_psnr(
            render_dict["images"], outputs_dict["images_gt"]
        ).mean()  # [B, V] -> float
        ...
        metric_logger.update(
            **{k: v.item() for k, v in outputs_dict.items() if "loss" in k}
        )
        ...
        if (
            log_writer is not None and (data_iter_step + 1) % accum_iter == 0
        ):  # Only True for main process
            ...
            if (
                epoch % config.train.vis_interval == 0
                or epoch == config.loss.render_epochs + 1
            ) and (data_iter_step + 1) == accum_iter:
                log_writer.add_images(
                    "GT_images",
                    outputs_dict["images_gt"][:, 0, :, :, :].detach().cpu(),
                    epoch_1000x,
                )
                log_writer.add_images(
                    "GT_depths",
                    outputs_dict["depths_gt"][:, 0, :, :, :].detach().cpu() / 2.0,
                    epoch_1000x,
                )  # normalize depth into [0, 1]
                log_writer.add_images(
                    "GT_alphas",
                    outputs_dict["masks_gt"][:, 0, :, :, :].detach().cpu(),
                    epoch_1000x,
                )
                log_writer.add_images(
                    "render_images",
                    render_dict["images"][:, 0, :, :, :].detach().cpu(),
                    epoch_1000x,
                )
                log_writer.add_images(
                    "render_depths",
                    render_dict["depths"][:, 0, :, :, :].detach().cpu() / 2.0,
                    epoch_1000x,
                )  # normalize depth into [0, 1]
                log_writer.add_images(
                    "render_alphas",
                    render_dict["alphas"][:, 0, :, :, :].detach().cpu(),
                    epoch_1000x,
                )
...
def evaluate(data_loader, model, device, config, epoch):
    ...
    for data_dict in metric_logger.log_every(data_loader, 50, header):
        ...
        if len(log_img_dict["GT"]) < 16:
            log_img_dict["GT"].append(
                outputs_dict["images_gt"][0, 0, :, :, :].detach().cpu().numpy()
            )
            log_img_dict["render_images"].append(
                render_dict["images"][0, 0, :, :, :].detach().cpu().numpy()
            )
            log_img_dict["render_alpha"].append(
                render_dict["alphas"][0, 0, :, :, :].detach().cpu().numpy()
            )
        ...
        metric_logger.meters["psnr"].update(
            psnr.mean().item(), n=B
        )  # use distribution mean not rigorous mean
    ...
    for k, v in log_img_dict.items():
        if len(v) > 0:
            log_img_dict[k] = np.stack(v, axis=0)
        else:
            log_img_dict[k] = np.zeros((0,), dtype=np.float32)
...
def inference(data_loader, model, device, config, args):
    ...
    for data_dict in metric_logger.log_every(data_loader, 50, header):
        ...
        with torch.amp.autocast(device_type="cuda", enabled=False):
            _, _, latent_mean, latent_logvar = model.encode(
                data_dict["rgb_inputs"], data_dict["points_input"], return_all=True
            )  # [B, num_lp, latent_dim]

        pbar = tqdm(enumerate(data_dict["idx"]))
        for b, idx in pbar:
            raw = data_loader.dataset.models[idx]
            if isinstance(raw, dict):
                model_id  = raw.get("model", raw)
                category  = raw.get("category", None)
            else:
                model_id = raw
                category = None

            dump_dir = os.path.join(dump_root, model_id)
            ...
            dump_dict = {
                "mean": latent_mean[b].detach().cpu(),
                "logvar": latent_logvar[b].detach().cpu(),
            }
