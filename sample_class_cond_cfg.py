...
if __name__ == "__main__":
    ...
    # render settings
    all_eval_pose = torch.load('./objv_eval_pose.pt')[:27]  # [40, 25]
    # intrinsic
    fov_rad = 0.691150367
    # extrinsic
    ...
    W2C_uniform = get_W2C_uniform(n_views=100, radius=1.75, device=device)  # [100, 4, 4]. gobjaverse radius seems in the range [1.5, 2.0]
    ...
    with torch.no_grad():
        ...
        null_token = clip.tokenize([''] * iters, truncate=True).to(device)  # [B==iters, 77]
        with torch.no_grad():
            null_features = clip_model.encode_text(null_token).float().unsqueeze(1)  # [B==iters, 1, 512]
        ...
        for cfg_scale in [3.5]:
            ...
            for seed in [args.seed]:
                ...
                for i in range(total//iters):
                    ...
                    text_token = clip.tokenize(texts[i*iters:(i+1)*iters], truncate=True).to(device)  # [B==iters, 77]
                    with torch.no_grad():
                        text_features = clip_model.encode_text(text_token).float().unsqueeze(1)  # [B==iters, 1, 512]

                    cond = torch.cat([text_features, null_features], dim=0)  # [2B, 1, 512]
                    batch_seeds = torch.arange(i*iters, (i+1)*iters).to(device) * 0 + seed
                    batch_seeds = torch.cat([batch_seeds, batch_seeds], dim=0)  # [2B,]

                    sampled_array = model.sample(cond=cond, batch_seeds=batch_seeds, cfg_scale=cfg_scale).float()  # [2B=2*iters, num_lp, latent_dim]
                    ...
                    for j in range(sampled_array.shape[0]):
                        ...
                        text_prefix = '_'.join(texts[i*iters:(i+1)*iters][j].split()).replace('/', '=')
                        ...
                        for v, img in enumerate(render_dict['images'][0]):
                            ...
                            iio.imwrite(f"{dump_dir}/images/seed{seed}_{i*iters+j:05d}-{text_prefix}-{v}.png", img_uint8)
