...
def get_run_env():
    try:
        ...
        # Handle cases where this might be run outside of a git repo context
        commit_hash = "N/A"
    git_env = f'Git Commit ID: {commit_hash}'
    run_command = ' '.join(sys.argv)
    return git_env, run_command


def backup_modified_files(backup_dir):
    ...
    for line in status_output.split('\n'):
        if line and not line.startswith("??"):
            # The first 3 characters: status + space
            file_path = line[3:]
