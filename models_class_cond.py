...
from models.dit_models import DiT
...
class LatentArrayTransformer(nn.Module):
    ...
    def __init__(self, in_channels, t_channels, n_heads, d_head,
                 depth=1, dropout=0., context_dim=None, out_channels=None, cond_dropout_prob=0.0):
        super().__init__()
    ...
    def forward(self, x, t, cond=None, **kwargs):
        # x: [B, N, d_x=64], t: [B,], cond: [B, 1, d=512]
        ...
        if self.training and self.cond_dropout_prob > 0:
            ...
            uncond = kwargs['uncond'].repeat(B, 1, 1)  # [B, 1, D]
            assert uncond.shape == cond.shape
            drop_ids = torch.rand(B, device=cond.device) < self.cond_dropout_prob
            y = torch.where(drop_ids[:, None, None], kwargs['uncond'], cond)
        else:
            y = cond
...
def edm_sampler(
    ...
    S_churn=0, S_min=0, S_max=float('inf'), S_noise=1, cfg_scale=None,
):
...
class EDMLoss:
    ...
    def __call__(self, net, inputs, labels=None, augment_pipe=None, **kwargs):
        ...
        D_yn = net(y + n, sigma, labels, **kwargs)
...
class EDMPrecond(torch.nn.Module):
    def __init__(self,
        ...
        self.model = LatentArrayTransformer(in_channels=channels, t_channels=256, n_heads=n_heads, d_head=d_head, depth=depth, cond_dropout_prob=0.1)
        # self.model = DiT(in_channels=channels, hidden_size=n_heads*d_head, depth=depth, num_heads=n_heads, cond_dropout_prob=0.1)
    ...
    @torch.no_grad()
    def sample(self, cond, batch_seeds=None, cfg_scale=None):
        # print(batch_seeds)
        if cond is not None:
        ...
        latents = rnd.randn([batch_size, self.n_latents, self.channels], device=device)  # latents[:B] == latents[B:]

        return edm_sampler(self, latents, cond, randn_like=rnd.randn_like, cfg_scale=cfg_scale)
...
def kl_d512_m512_l16_d24_edm():
    model = EDMPrecond(n_latents=512, channels=16, depth=24)
    return model
...
def kl_d512_m512_l64_d28_edm():
    model = EDMPrecond(n_latents=512, channels=64, depth=28)
    return model
...
def kl_d512_m512_l64_d12_edm():
    model = EDMPrecond(n_latents=512, channels=64, depth=12)
    return model
