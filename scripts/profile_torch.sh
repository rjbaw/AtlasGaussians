...
python main_ae_profile_torch.py \
  ...
  --resume "${resume_path}" > profile_torch_vae_log.txt
...
python main_class_cond_profile_torch.py \
  ...
  --warmup_epochs 50 \
  --lr 1e-4 \
  --latent_dir "${full_log_dir}/inference/latents/epoch_1000" > profile_torch_ldm_log.txt
...
CUDA_VISIBLE_DEVICES=0 python sample_class_cond_cfg_profile_torch.py \
	...
	--seed 5 > profile_torch_infer_log.txt
