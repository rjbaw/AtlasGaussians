...
nsys profile -o vae --trace=cuda,nvtx,cublas,cusparse \
    python main_ae_profile_nvtx.py \
  --distributed \
  --config "${full_config}" \
  --log_dir "${full_log_dir}" \
  --resume "${resume_path}" > profile_nvtx_vae_log.txt

nsys profile -o ldm --trace=cuda,nvtx,cublas,cusparse \
  ...
  --warmup_epochs 50 \
  --lr 1e-4 \
  --latent_dir "${full_log_dir}/inference/latents/epoch_1000" > profile_nvtx_ldm_log.txt

CUDA_VISIBLE_DEVICES=0 \
        nsys profile -o inference --trace=cuda,nvtx,cublas,cusparse \
        ...
        --seed 5 > profile_nvtx_infer_log.txt
