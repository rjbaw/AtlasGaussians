...
class Objaverse(data.Dataset):
    def __init__(self, split, data_root, num_views=None, img_size_input=224, img_size_render=512, replica=16):
    ...
    def __getitem__(self, idx):
        ...
        for ind in ids:
            #### Read raw RGB
            rgba = iio.imread(f"{self.img_root}/{model}/{ind:05d}/{ind:05d}.png") / 255.0  # [H, W, 4]
            rgb = rgba[:, :, :3]  # [H, W, 3]
            ...
            jdata = json.load(open(f"{self.img_root}/{model}/{ind:05d}/{ind:05d}.json"))
            ...
            world2cam[:3, :3] = world2cam[:3, :3] @ rotation_matrices_z_ccw[self.label_view[model]].T

            ##### Read depth
            depth = read_gobjaverse_depth(f"{self.img_root}/{model}/{ind:05d}/{ind:05d}_nd.exr", cam2world[:3, 3:])  # [H, W, 1]
            ...
            if self.img_size_input != width:
                rgb_input  = cv2.resize(rgb_input,  (self.img_size_input, self.img_size_input), interpolation=cv2.INTER_LANCZOS4)
        ...
        points_all = torch.stack((points_all[:, 0], -points_all[:, 2], points_all[:, 1]), dim=-1)
        # Rotate point clouds
        ...
        data_dict = {'idx': idx, 'rgb_inputs': rgb_inputs,
                     'rgb_gt': rgb_gt, 'depth_gt': depth_gt, 'mask_gt': mask_gt,
                     'world2cams': world2cams, 'fov_rad': fov_rad, 'img_size_render': self.img_size_render, 
...
def build_dataset(split, cfg):
    dataset = Objaverse(split, cfg.data_root, num_views=cfg.num_views,
